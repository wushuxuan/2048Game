...
exports.save = async (request, response) => {
    ...
    try {
        ...
        task.creator = creator.email;
        task.creatorUser = creator.username;
        const item = await taskService.get(task.creator);
        if (item) {
            var score = item.score
            if (parseInt(item.score) < parseInt(task.score)) {
...
exports.get = (request, response) => {
    ...
    const promise = taskService.getByIdEmail({
        taskId,
        email
    });
    promise
...
exports.oneDay = async (request, response) => {
    ...
    try {
        ...
        const tasks = await taskService.oneDay({
            day,
            email
        });
        response.status(200);
